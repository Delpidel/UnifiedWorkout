            ...
            <v-form @submit.prevent="submitForm">
              <v-row>
                <v-col cols="12" md="12">
                  ...
                  <v-select v-model="selectedExercise" label="Exercício" :items="getExerciseDescription()"></v-select>
                </v-col>
                <v-col cols="12" md="4">
                  <p class="pb-2">Repetições</p>
                  <v-text-field v-model="repetitions" label="Repetições" type="number" min="1" required></v-text-field>
                </v-col>
                <v-col cols="12" md="4">
                  <p class="pb-2">Quilos</p>
                  <v-text-field v-model="weight" label="Quilos" type="number" required></v-text-field>
                </v-col>
                <v-col cols="12" md="4">
                  <p class="pb-2">Pausa</p>
                  <v-text-field v-model="breakTime" label="Pausa" type="time" required></v-text-field>
                </v-col>
                <v-col cols="12" md="12">
  ...
  methods: {
    ...
    getExerciseId(description) {
      const exercise = this.exercises.find(
        (item) => item.exercise_description === description
      );

      return exercise.id;
    },
    submitForm() {
        this.selectedExerciseId = this.getExerciseId(
        this.exercises.description
      );
      const formData = {
      ...
      axios.post(`http://localhost:3000/workouts`, formData)
        .then(response => {
